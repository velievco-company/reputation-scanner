...
brightdata_available = search_engine._get_secret(“BRIGHTDATA_API_KEY”) is not None
...
st.success(“✅ Bright Data подключен”)
else:
st.info(“ℹ️ Bright Data не подключен — используется DuckDuckGo”)
with status_cols[2]:
if brave_available:
st.success(“✅ Brave Search подключен”)
else:
st.info(“ℹ️ Brave не подключен (резервный источник)”)
with status_cols[3]:
if newsapi_available:
st.success(“✅ NewsAPI подключен”)
else:
st.info(“ℹ️ NewsAPI не подключен — новости только через поиск”)
